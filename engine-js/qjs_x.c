...
static const char* PRELUDE =
    ...
    "   'box','slider','progress']"
    "    .forEach(function(t){ ui[t] = function(p){"
    "      /* string shorthand on ui.text only: ui.text('hi') ≡ ui.text({text:'hi'}) */"
    "      var p0 = (t === 'text' && typeof p === 'string') ? { text: p } : p;"
    "      var a = [t, p0]; for (var i = 1; i < arguments.length; i++) a.push(arguments[i]);"
    "      return node.apply(null, a); }; });"
    "  ui.node = node;"
    "  globalThis.ui = ui;"
    "})();";
