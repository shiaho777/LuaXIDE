...
调试不再强制树遍历:`debug_enabled` 时 VM 按帧维护"上次触发行",每当指令行号变化或控制流回跳(循环迭代)即调 `vm_line_hit`。寄存器驻留 proto 由 `Proto.locm`(编译期记录每条局部声明的 name/reg/pc)合成快照 Env;capmode proto 的局部量本就在 `cur` 中,调试器所见即所得。暂停返回后快照写回寄存器,eval 赋值与树遍历路径行为一致。
```
...
所有语句/表达式形态均可编译(含变长参数);调试会话(断点、条件断点、logpoint、step/step-out、break-on-error、locals 查看、帧内 eval)经上述按帧行号钩子直接在 VM 上运行。剩余回退仅 `LUAX_NO_BC` 与超限情形(u16 常量索引、i16 跳转增量、200 寄存器)。

注意:快照中的局部量取 `pc <=` 暂停点处该名字的最近一次声明——已出作用域的名字在寄存器被复用前可能仍可见;eval 写回只落在当前生效的声明上。
