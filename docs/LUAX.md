...
- 是 LuaXIDE 三语言平台(Lua / JavaScript / Python)的**参考语言** —— 跨语言契约见 [PLATFORM_ABI.md](PLATFORM_ABI.md)
...
通用:`key`/`id` 作为 diff 身份(影响动画)。序列化为 `{type, props, children}` JSON;props 键序为哈希表槽序(测试断言须用子串匹配)。完整渲染契约(两模块一致的权威属性表)随 [PLATFORM_ABI.md](PLATFORM_ABI.md) §6 演进。
